...
# Invoked explicitly as: run-as com.termux $PREFIX/bin/bash <this-script>.
# It runs only the QEMU UEFI collector on a disposable FAT image.
set -euo pipefail
export HOME=/data/user/0/com.termux/files/home
export PREFIX=/data/data/com.termux/files/usr
export PATH="$PREFIX/bin:/system/bin"

# A run directory is explicit to prevent replacing any earlier evidence.
ROOT=/sdcard/Download/qemu-contract-20260913-v4
...
[[ -r "$CODE" && -r "$VARS_TEMPLATE" && -r "$ESP" ]] || {
  echo 'missing QEMU firmware or disposable ESP' > "$STDERR"
  exit 2
}
[[ ! -e "$VARS" && ! -e "$SERIAL" && ! -e "$STDERR" && ! -e "$STATUS" ]] || {
  echo 'refusing to overwrite a prior contract-run artifact' >&2
  exit 3
}
