...
//
// The AVF test VM deliberately has no writable UEFI variable store.  This is
// a one-shot development path, not a replacement for normal BDS policy: find
// the first filesystem which really contains the standard removable-media
// loader and start it with a complete device path.
//
STATIC
EFI_STATUS
AvfConnectBootMediaChildren (
  VOID
  )
{
  EFI_STATUS  Status;
  ...
  Print (L"AVF_BDS_BLOCK_SCAN status=%r count=%u\n", Status, HandleCount);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  for (Index = 0; Index < HandleCount; ++Index) {
    ...
    Print (L"AVF_BDS_CONNECT_BLOCK[%u]=%r\n", Index, Status);
  }
...
STATIC
EFI_STATUS
AvfConnectVirtioBlockControllers (
  VOID
  )
{
  EFI_STATUS           Status;
  ...
  Print (L"AVF_BDS_PCI_SCAN status=%r count=%u\n", Status, HandleCount);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  for (Index = 0; Index < HandleCount; ++Index) {
    ...
    if (!EFI_ERROR (Status) && Id == 0x10421AF4) {
      Status = gBS->ConnectController (Handles[Index], NULL, NULL, FALSE);
      Print (L"AVF_BDS_CONNECT_VBLK[%u]=%r\n", Index, Status);
      break;
    }
  }
  ...
  )
{
  ...
  Print (L"AVF_BDS_FS_SCAN status=%r count=%u\n", Status, HandleCount);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  for (Index = 0; Index < HandleCount; ++Index) {
    ...
    if (EFI_ERROR (Status)) {
      Print (L"AVF_BDS_FS[%u]_OPEN_VOLUME=%r\n", Index, Status);
      continue;
    }
    Status = Root->Open (
                     Root,
    ...
    Root->Close (Root);
    if (EFI_ERROR (Status)) {
      Print (L"AVF_BDS_FS[%u]_NO_BOOTAA64=%r\n", Index, Status);
      continue;
    }
    File->Close (File);

    Path = FileDevicePath (Handles[Index], EFI_REMOVABLE_MEDIA_FILE_NAME);
    if (Path == NULL) {
    ...
    FreePool (Path);
    if (!EFI_ERROR (Status)) {
      Print (L"AVF_BDS_START_IMAGE \\EFI\\BOOT\\BOOTAA64.EFI fs=%u\n", Index);
      EfiBootManagerBoot (&Option);
      Print (L"AVF_BDS_START_IMAGE_RETURN status=%r\n", Option.Status);
  ...
  )
{
  ...
  //
  // The AVF image has no variable store and the generic global connection
  // blocks after the PCI rescan.  Try the already-enumerated filesystem
  // handles first, before entering that policy.
  //
  AvfConnectVirtioBlockControllers ();
  AvfConnectBootMediaChildren ();
  AvfBootRemovableMedia ();
  // Keep the normal policy helper linked for the stock path; it is deliberately
  // not executed by this no-variable-store AVF test configuration.
  if (FALSE) {
    BootDiscoveryPolicyHandler ();
  }
